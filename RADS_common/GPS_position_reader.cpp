...
namespace Readings {
    namespace GPS_position {
        ...
        float GPS_position_reader::last_alt = (static_cast<float> (rand() / static_cast<float> (RAND_MAX / 10000)) + 30000);
        ...
        void GPS_position_reader::read() {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                if (i % 3 == 0) {
                    random_alt += static_cast<float> (rand() / static_cast<float> (RAND_MAX/30));  // Add 0 - 30 for altitude
                }
                else {
                    random_alt -= static_cast<float> (rand() / static_cast<float> (RAND_MAX / 100));  // Decrease altitude by 0 - 100
                }
            }
